...
public:
   ...
   * @param maxDistance
   */
  std::vector<Vertex<LocalizedRangeScan> *> FindNearByVertices(
...
private:
  ...
  inline Vertex<LocalizedRangeScan> * GetVertex(LocalizedRangeScan * pScan)
  {
    ...
    std::map<int, Vertex<LocalizedRangeScan> *>::iterator it = m_Vertices[rName].find(
      pScan->GetStateId());
    if (it != m_Vertices[rName].end()) {
      return it->second;
    } else {
...
public:
  ...
  {
    std::cout <<
      "ModifyNode method not implemented for this solver type. Manual loop closure unavailable." <<
      std::endl;
  }
  /**
   * Get node's yaw
...
public:
  ...
  static CorrelationGrid * CreateGrid(
    kt_int32s width,
    kt_int32s height,
    ...
    kt_double smearDeviation)
  {
    ...
    CorrelationGrid * pGrid = new CorrelationGrid(width, height, borderSize, resolution,
        smearDeviation);
  ...
  inline void SmearPoint(const Vector2<kt_int32s> & rGridPoint)
  {
    ...
    for (kt_int32s j = -halfKernel; j <= halfKernel; j++) {
      kt_int8u * pGridAdr =
        GetDataPointer(Vector2<kt_int32s>(rGridPoint.GetX(), rGridPoint.GetY() + j));
...
protected:
   ...
   */
  CorrelationGrid(
    kt_int32u width, kt_int32u height, kt_int32u borderSize,
  ...
  virtual void CalculateKernel()
  {
    ...
    if (!math::InRange(m_SmearDeviation, MIN_SMEAR_DISTANCE_DEVIATION,
